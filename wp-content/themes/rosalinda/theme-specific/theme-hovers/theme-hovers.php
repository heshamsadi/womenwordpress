...
if ( ! function_exists( 'rosalinda_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'rosalinda_hovers_theme_setup3', 3 );
	function rosalinda_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		rosalinda_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'rosalinda' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'rosalinda' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'rosalinda' ),
						'slide_left'   => esc_html__( 'Slide from Left', 'rosalinda' ),
						'slide_right'  => esc_html__( 'Slide from Right', 'rosalinda' ),
						'slide_top'    => esc_html__( 'Slide from Top', 'rosalinda' ),
						'slide_bottom' => esc_html__( 'Slide from Bottom', 'rosalinda' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'rosalinda' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'rosalinda' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'rosalinda' ),
					),
					'options'  => rosalinda_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'rosalinda_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'rosalinda_hovers_theme_setup9', 9 );
	function rosalinda_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'rosalinda_hovers_frontend_scripts', 1010 );
		add_filter( 'rosalinda_filter_localize_script', 'rosalinda_hovers_localize_script' );
		add_filter( 'rosalinda_filter_merge_scripts', 'rosalinda_hovers_merge_scripts' );
		add_filter( 'rosalinda_filter_merge_styles', 'rosalinda_hovers_merge_styles' );
		add_filter( 'rosalinda_filter_merge_styles_responsive', 'rosalinda_hovers_merge_styles_responsive' );
		add_filter( 'rosalinda_filter_get_css', 'rosalinda_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'rosalinda_hovers_frontend_scripts' ) ) {
	
	function rosalinda_hovers_frontend_scripts() {
		if ( rosalinda_is_on( rosalinda_get_theme_option( 'debug_mode' ) ) ) {
			$rosalinda_url = rosalinda_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $rosalinda_url ) {
				wp_enqueue_script( 'rosalinda-hovers', $rosalinda_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'rosalinda_hovers_merge_styles' ) ) {
	
	function rosalinda_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'rosalinda_hovers_merge_styles_responsive' ) ) {
	
	function rosalinda_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'rosalinda_hovers_localize_script' ) ) {
	
	function rosalinda_hovers_localize_script( $arr ) {
		$arr['button_hover'] = rosalinda_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'rosalinda_hovers_merge_scripts' ) ) {
	
	function rosalinda_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'rosalinda_hovers_add_icons' ) ) {
	function rosalinda_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_url( $no_link ); ?>" <?php rosalinda_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php rosalinda_show_layout($target); ?> aria-hidden="true" class="icon-link"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php rosalinda_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php rosalinda_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php rosalinda_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'rosalinda_hovers_get_css' ) ) {
	
	function rosalinda_hovers_get_css( $css, $args ) {
