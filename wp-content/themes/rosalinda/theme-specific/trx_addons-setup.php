...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'rosalinda_trx_addons_theme_specific_setup1' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'rosalinda_trx_addons_theme_specific_components' );
	function rosalinda_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'rosalinda_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'rosalinda_trx_addons_theme_specific_setup1', 1 );
	function rosalinda_trx_addons_theme_specific_setup1() {
		if ( rosalinda_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'rosalinda_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'rosalinda_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'rosalinda_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'rosalinda_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'rosalinda_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'rosalinda_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'rosalinda_trx_addons_widgets_list' );
		}
	}
}

// CV
if ( ! function_exists( 'rosalinda_trx_addons_cv_enable' ) ) {
	
	function rosalinda_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return ROSALINDA_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'rosalinda_trx_addons_demo_enable' ) ) {
	
	function rosalinda_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'rosalinda_trx_addons_edd_themes_market_enable' ) ) {
	
	function rosalinda_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'rosalinda_trx_addons_api_list' ) ) {
	
	function rosalinda_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'rosalinda_trx_addons_cpt_list' ) ) {
	
	function rosalinda_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'rosalinda_trx_addons_sc_list' ) ) {
	
	function rosalinda_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'rosalinda_trx_addons_widgets_list' ) ) {
	
	function rosalinda_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'rosalinda_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'rosalinda_trx_addons_menu_cache' );
	function rosalinda_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'rosalinda_trx_addons_localize_script' ) ) {
	...
	function rosalinda_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = rosalinda_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'rosalinda_trx_addons_sc_type', 10, 2 );
	function rosalinda_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = rosalinda_array_merge( $list, rosalinda_get_list_blog_styles( false, 'sc' ) );
		}
        if ( 'trx_sc_title' == $sc ) {
            $list['icon'] = 'Icon';
            $list['extra'] = 'Extra';
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'rosalinda_trx_addons_sc_prepare_atts', 10, 2 );
	function rosalinda_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = rosalinda_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = rosalinda_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'rosalinda_trx_addons_sc_atts', 10, 2 );
	function rosalinda_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'rosalinda_trx_addons_sc_map', 10, 2 );
	function rosalinda_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'rosalinda' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'rosalinda' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( rosalinda_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'rosalinda' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'rosalinda' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( rosalinda_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'rosalinda_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'rosalinda_trx_addons_sow_map', 10, 2 );
	function rosalinda_trx_addons_sow_map( $params, $sc ) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label'       => esc_html__( 'Color style', 'rosalinda' ),
				'description' => wp_kses_data( __( 'Select color style to decorate this block', 'rosalinda' ) ),
				'options'     => rosalinda_get_list_sc_color_styles(),
				'default'     => 'default',
				'type'        => 'select',
			),
		);
		if ( in_array( $sc, array( 'trx_sc_button' ) ) ) {
			rosalinda_array_insert_after( $params, 'size', $param );
		} elseif ( in_array(
			$sc, array(
				'trx_sc_action',
				'trx_sc_blogger',
			...
			rosalinda_array_insert_after( $params, 'title_align', $param );
		}
		return $params;
	}
}
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'rosalinda_trx_addons_sc_output', 10, 4 );
	function rosalinda_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! rosalinda_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'rosalinda_trx_addons_sc_item_title_tag' );
	function rosalinda_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'rosalinda_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'rosalinda_trx_addons_sc_item_button_args', 10, 3 );
	function rosalinda_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'rosalinda_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'rosalinda_trx_addons_slider_title', 10, 2 );
	function rosalinda_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'rosalinda_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'rosalinda_trx_addons_sc_googlemap_styles' );
	function rosalinda_trx_addons_sc_googlemap_styles( $list ) {
		$list['light'] = esc_html__( 'Light', 'rosalinda' );
		return $list;
	}
}
...
if ( ! function_exists( 'rosalinda_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'rosalinda_trx_addons_tiny_mce_style_formats' );
	function rosalinda_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'rosalinda' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'rosalinda' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'rosalinda_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'rosalinda_filter_allow_override_header_image', 'rosalinda_trx_addons_allow_override_header_image' );
	function rosalinda_trx_addons_allow_override_header_image( $allow ) {
		return rosalinda_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( ! function_exists( 'rosalinda_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'rosalinda_trx_addons_thumb_size', 10, 2 );
	function rosalinda_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'rosalinda_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'rosalinda_trx_addons_meta_box_fields', 10, 2 );
	function rosalinda_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'rosalinda' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'rosalinda' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
