/* global rosalinda_color_schemes, rosalinda_dependencies, Color */
...
( function( api ) {
	...
	for (var i in rosalinda_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'rosalinda-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'rosalinda-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'rosalinda-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + rosalinda_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( rosalinda_customizer_vars['msg_reset_confirm'] )) {
			...
			if (rosalinda_customizer_vars && rosalinda_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( rosalinda_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', rosalinda_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', rosalinda_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					rosalinda_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							rosalinda_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			rosalinda_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			rosalinda_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof rosalinda_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-rosalinda-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-rosalinda-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in rosalinda_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-rosalinda-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-rosalinda-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-rosalinda-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'rosalinda-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in rosalinda_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (rosalinda_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = rosalinda_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = rosalinda_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function rosalinda_customizer_get_field_value(fld) {
	...
	function rosalinda_customizer_check_dependencies(cont) {
		cont.find( '.customize-control' ).each(
			function() {
				...
				var fld    = null, val = '', i;
				var depend = false;
				for (fld in rosalinda_dependencies) {
					if (fld == id) {
						depend = rosalinda_dependencies[id];
						break;
					}
				}
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = rosalinda_customizer_get_field_value( fld );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'rosalinda_options_no_use' );
					} else {
						ctrl.hide().addClass( 'rosalinda_options_no_use' );
					}
				}
				...
				if (false && id == 'color_scheme') {
					...
					if (fld.length > 0) {
						val     = rosalinda_customizer_get_field_value( fld );
						var num = 0;
						for (i in rosalinda_color_schemes) {
							num++;
							if (i == val) {
								break;
							}
	...
	function rosalinda_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( rosalinda_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					rosalinda_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in rosalinda_theme_fonts) {
					for (rule in rosalinda_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							rosalinda_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
			...
			if (processed || forceUpdateCSS) {
				rosalinda_customizer_update_css();
			}
		}
	...
	// Actions buttons
	function rosalinda_customizer_actions(e) {
	...
	function rosalinda_customizer_update_theme_vars(opt, value) {
		rosalinda_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function rosalinda_customizer_update_theme_fonts(opt, rule, value) {
		rosalinda_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function rosalinda_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= rosalinda_customizer_vars['max_load_fonts']; i++) {
		...
		for (tag in rosalinda_theme_fonts) {
	...
	function rosalinda_customizer_update_css() {
		...
		var vars = rosalinda_customizer_add_theme_vars( rosalinda_theme_vars );

		// Make styles and add its to the css
		css += rosalinda_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = rosalinda_customizer_add_theme_fonts( rosalinda_theme_fonts );

		// Make styles and add its to the css
		css += rosalinda_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in rosalinda_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( rosalinda_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in rosalinda_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in rosalinda_color_schemes[scheme].colors) {
				colors[i] = rosalinda_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = rosalinda_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-rosalinda-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function rosalinda_customizer_add_theme_colors(colors) {
		if (rosalinda_additional_colors) {
			var clr = '', v = '';
			for (var k in rosalinda_additional_colors) {
				v   = rosalinda_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + rosalinda_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + rosalinda_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + rosalinda_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function rosalinda_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']   = rosalinda_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']        = rosalinda_customizer_prepare_css_value( vars['page'] );
				rez['sidebar']     = rosalinda_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']         = rosalinda_customizer_prepare_css_value( vars['gap'] );
				rez['content']     = rosalinda_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['sidebar_gap'] = rosalinda_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc'] = vars['sidebar'] / vars['page'];
				rez['gap_prc']     = vars['gap'] / vars['page'];
				rez['page_boxed']  = rosalinda_customizer_prepare_css_value( vars['page'] + 2 * 60 );
				rez['page_1_1']    = rosalinda_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']    = rosalinda_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']    = rosalinda_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']    = rosalinda_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']    = rosalinda_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']    = rosalinda_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins        = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = rosalinda_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = rosalinda_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function rosalinda_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function rosalinda_customizer_prepare_html_value(val) {
