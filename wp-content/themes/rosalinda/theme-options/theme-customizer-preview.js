...
( function( $ ) {
	...
	var $style = $( '#rosalinda-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="rosalinda-customizer-inline-css" />',
			$theme_css      = $( '#rosalinda-main-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#rosalinda-main-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#rosalinda-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( rosalinda_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( rosalinda_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( rosalinda_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof rosalinda_color_schemes[scheme] != 'undefined' ? rosalinda_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : rosalinda_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'rosalinda-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
