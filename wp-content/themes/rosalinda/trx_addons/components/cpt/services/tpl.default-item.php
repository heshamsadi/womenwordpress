...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			?>><?php
				if (!empty($svg)) {
					...
					?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'rosalinda'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			echo !empty($link) 
				? '</a>' 
				: '</span>';
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'rosalinda'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
		} else {
    ...
    <div class="sc_services_item_footer">
        <?php
        // Price
        $price_services = $meta['price'];
        if ($price_showed && !empty($meta['price'])) {
            ?><div class="sc_services_item_price"><?php trx_addons_show_layout($price_services); ?></div><?php
        }
