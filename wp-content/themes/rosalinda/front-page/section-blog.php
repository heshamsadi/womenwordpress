<div class="front_page_section front_page_section_blog<?php
	$rosalinda_scheme = rosalinda_get_theme_option( 'front_page_blog_scheme' );
	if ( ! rosalinda_is_inherit( $rosalinda_scheme ) ) {
		echo ' scheme_' . esc_attr( $rosalinda_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( rosalinda_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$rosalinda_css      = '';
		$rosalinda_bg_image = rosalinda_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$rosalinda_anchor_icon = rosalinda_get_theme_option( 'front_page_blog_anchor_icon' );
	$rosalinda_anchor_text = rosalinda_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( rosalinda_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$rosalinda_css      = '';
			$rosalinda_bg_mask  = rosalinda_get_theme_option( 'front_page_blog_bg_mask' );
			$rosalinda_bg_color_type = rosalinda_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $rosalinda_bg_color_type ) {
				$rosalinda_bg_color = rosalinda_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$rosalinda_caption = rosalinda_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $rosalinda_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $rosalinda_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $rosalinda_caption, 'rosalinda_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$rosalinda_description = rosalinda_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $rosalinda_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $rosalinda_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $rosalinda_description ), 'rosalinda_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! rosalinda_exists_trx_addons() ) {
					...
					rosalinda_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
