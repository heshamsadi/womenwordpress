...
/* =============================================================================
   GLOBAL HELPERS (used by your custom article template part)
   ============================================================================= */

/**
 * Minimal breadcrumbs (Home › Category › Title)
 * @return string
 */
function rosalinda_child_breadcrumbs() {
	$parts = array( '<a href="' . esc_url( home_url( '/' ) ) . '">Home</a>' );
	$cats  = get_the_category();
	if ( ! empty( $cats ) ) {
		$c = $cats[0];
		$parts[] = '<a href="' . esc_url( get_category_link( $c->term_id ) ) . '">' . esc_html( $c->name ) . '</a>';
	}
...
/**
 * Approximate reading time (defaults to 200 wpm)
 * @param int $wpm
 * @return string
 */
function rosalinda_child_reading_time( $wpm = 200 ) {
	$content = get_post_field( 'post_content', get_the_ID() );
	$words   = str_word_count( wp_strip_all_tags( (string) $content ) );
	$mins    = max( 1, (int) ceil( $words / max( 1, (int) $wpm ) ) );
	return sprintf( _n( '%d min read', '%d mins read', $mins, 'rosalinda-child' ), $mins );
}

/**
 * Optional ad hook outputs — replace with your AdSense code when ready
 */
add_action( 'rosalinda_child_ad_before_content', function () {
	// echo '<div class="ad-slot ad-top">YOUR ADSENSE CODE</div>';
} );
add_action( 'rosalinda_child_ad_after_content', function () {
	// echo '<div class="ad-slot ad-bottom">YOUR ADSENSE CODE</div>';
} );
add_action( 'rosalinda_child_before_article', function () {
	// echo '<div class="ad-slot ad-before-article">YOUR ADSENSE CODE</div>';
} );
add_action( 'rosalinda_child_after_article', function () {
	// echo '<div class="ad-slot ad-after-article">YOUR ADSENSE CODE</div>';
} );

/**
 * Legacy functions (kept global to avoid namespace issues)
 */
...
/* =============================================================================
   NAMESPACED FEATURES (patterns, shortcodes, schema)
   ============================================================================= */
 ...
 *
 * Usage: [ad slot="top|mid|end|sidebar|footer"]
 * @param array $atts
 * @return string
 */
function shortcode_ad( $atts ) {
	...
	$allowed = array( 'top', 'mid', 'end', 'sidebar', 'footer' );
	$slot    = in_array( $atts['slot'], $allowed, true ) ? $atts['slot'] : 'mid';
